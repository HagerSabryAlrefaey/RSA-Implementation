...
namespace RSA_Project
{
    class BigInt
    {
        ...
        {
            //Initializing values that will be needed in the function.

            //array that has Qoutient(index 0) and Remainder(index 1) 
            BigInt[] QandR = new BigInt[2];

            //numbers 0 and 1 but is BigInt data type, to be able to use them.
            ...
            //if val1 is smaller than val2 (Base Case)
            ...
            //calling the Div function with 2*val2 (Recursion)
            QandR = Div(val1, Add(val2, val2));

            //reassigning Qoutient with 2*Qoutient
            QandR[0] = Add(QandR[0], QandR[0]);

            //goes here if remainder is smaller than val2 
            if (checkBigger(QandR[1], val2) == 0)
                return QandR;

            //adding 1 to Qoutient and subtracting remainder from val2
        ...
        public BigInt Power(BigInt val, BigInt pow, BigInt M)
        {
            ...
            if (checkBigger(pow, one) == 2)
                return val;

            newpow = Power(val, Div(pow, two).ElementAt(0), M);

            if (checkBigger(Div(pow, two).ElementAt(1), zero) == 2)
                newpow = Mul(newpow, newpow);
            else
            {
                newpow = Mul(Mul(newpow, newpow), val);
            }
            return Div(newpow, M).ElementAt(1);
        }

        //To check whether val1 is bigger(returns 1) OR smaller(returns 0) OR equal(returns 2) to val2  
        private int checkBigger(BigInt val1, BigInt val2)
        {
            //removing zeroes on the left of the values. left_zeroes is O(N)
            val1 = left_Zeros(val1);
            val2 = left_Zeros(val2);

            //if No. of digits in val1 is more than No. of digits in val2, then val1 is bigger.
            if (val1.Digits.Count > val2.Digits.Count)
            {
                return 1;
            }

            //goes here if both values are same size.
            else if (val1.Digits.Count == val2.Digits.Count)
            {

                //checking every number digit in both values, from left to right.
            ...
            //goes here if val1 size is smaller than val2 size
            else
                return 0;

            //if they are equal in size AND equal in value.
            return 2;
        }
